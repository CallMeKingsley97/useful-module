...
本 skill 现将中小尺寸防重叠规则提升为统一标准。关键认知只有一条：`systemMedium` 增加的主要是宽度，不是高度，因此不能把它当成“`systemSmall` 再多塞几行”的版本。

后续所有主屏小组件默认遵守以下标准：

- `systemSmall` 最多只保留 `5` 个视觉块：
  - 标题
  - 主状态或主指标
  - 结果摘要，最多 `2` 行
  - 单条元信息，如连签或最近时间，二选一
  - 短尾注
- `systemMedium` 最多只保留 `4` 到 `5` 条业务信息：
  - 标题
  - 分隔
  - 状态
  - 结果，最多 `2` 行
  - `1` 到 `2` 条元信息
  - 可选短尾注
- `systemLarge` 才允许承载扩展说明、操作提示、脚本名、定时信息，但仍必须保持单层平铺
...
3. 单条高价值元信息
4. 短尾注
5. 操作提示、脚本名、定时说明、调试信息

强制规则：

- `结果` 之外的正文默认都必须单行，并配置 `maxLines: 1`
- 中小尺寸必须避免重复信息，正文与 footer 不得同时重复显示时间、说明或状态
- 中小尺寸默认禁止展示完整脚本名、冗长操作说明、长调度文案
- 内容超载时只能按以下顺序处理：缩短文案 → 删除低优先级行 → 下放到 `systemLarge`
- 不允许通过持续减小字号去硬塞全部信息
- `systemSmall` 或 `systemMedium` 一旦已经发生重叠或截断，直接回退平铺模板，不再继续修补复杂结构

验收基线：

- 至少覆盖 `success`、`failed`、`pending` 与一组长错误文案状态
- `systemSmall` 与 `systemMedium` 中，标题、主状态、结果摘要必须始终可见
- 不得出现底部被裁切、横向标签挤爆、footer 与正文争抢空间的情况
