...
- `systemSmall` 最多只保留 `5` 个视觉块：
  ...
  - 单条元信息，如连签或最近时间，二选一
  - 短尾注
- `systemSmall` 的尾注默认按“单个短意图”处理，不承载复合信息：
  - 只保留一个目标：下次时间 / 可重试 / 可补签 / 需更新授权
  - 禁止把“下次时间 + 倒计时 + 历史摘要”拼在同一行
  - 若主区已展示连签，则尾注不再重复历史或额外时间解释
  - 默认将尾注控制在 `6~8` 个中文字符可读范围内，例如：`明09:00`、`可重试`、`可补签`
...
- `systemSmall` 若出现底部省略号，优先删减尾注语义，而不是继续压缩字号或把正文挤短
...
- small 看似是方块，但实际可安全承载的文本宽度明显小于 medium，不要把它当成“窄一点的 medium”
- 底部尾注只适合单个短意图，不适合“时间 + 倒计时 + 历史摘要”的拼接串
- 如果主摘要已经包含时间语义，尾注应退化为动作词或更短时间，如：`可重试`、`可补签`、`明09:00`
- 尾注一旦出现省略号，说明信息层级已经错位，应先删信息而不是继续缩放
