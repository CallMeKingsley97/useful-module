...
- 只允许承载一个短意图：下次时间 / 可重试 / 可补签 / 需更新授权
- 不允许把“下次时间 + 倒计时 + 历史摘要”压成一行
- 若主区已展示时间或连签，尾注应退化为更短的动作词或纯时间点
- 实战上把尾注文案收敛到 `6~8` 个中文字符最稳，例如：`明09:00`、`可重试`、`可补签`
...
- small 出现底部省略号时，优先删除尾注中的次要语义，而不是继续缩字号
...
- 元信息只保留 1 条
- 底部状态保持极简
- 不要把 small 当成“缩小版 medium”；它能安全承载的文本宽度更接近“短句卡片”而不是“信息列表”
- 尾注如果开始出现省略号，说明不是截断策略问题，而是信息分层错了，应先删信息
